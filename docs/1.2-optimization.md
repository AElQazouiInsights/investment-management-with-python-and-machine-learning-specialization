...
def build_portfolios_with_gmv_msr():
    ...
    4. Constructs the efficient frontier based on the generated portfolios.
    5. Prepares scatter and line data for visualization.
    6. Adds GMV and MSR data points.
    7. Compiles all data into a JSON format suitable for ECharts.
    8. Outputs a summary table of GMV and MSR portfolios.
    """
    # Step 1: Load Stock Price Data
    stocks = pok.get_stock_dynamic()
    ...
    # Step 5: Identify GMV (lowest volatility) and MSR (highest Sharpe Ratio) portfolios
    gmv_port = portfolios.iloc[portfolios["volatility"].idxmin()]  # Portfolio with the lowest volatility
    msr_port = portfolios.iloc[portfolios["sharpe"].idxmax()]      # Portfolio with the highest Sharpe Ratio
    
    # Step 6: Construct the Efficient Frontier DataFrame
    df_frontier = pok.efficient_frontier(50, daily_rets, cov_rets, periods_per_year)
    # 'df_frontier' contains columns like ["volatility", "return"] representing points on the efficient frontier
    
    # Step 7: Prepare Scatter Data for the Portfolios
    # Each portfolio is represented as a list containing [volatility, return, sharpe_ratio]
    scatter_data = []
    for _, row in portfolios.iterrows():
        scatter_data.append([
    ...
    # Step 8: Prepare Line Data for the Efficient Frontier
    # Each point on the efficient frontier is represented as a list containing [volatility, return]
    line_data = []
    for _, row in df_frontier.iterrows():
        line_data.append([
    ...
    # Step 9: Prepare Single-Point Data for GMV and MSR Portfolios (Including Sharpe Ratio)
    gmv_data = [[gmv_port["volatility"], gmv_port["return"], gmv_port["sharpe"]]]   # GMV as a single point with Sharpe Ratio
    msr_data = [[msr_port["volatility"], msr_port["return"], msr_port["sharpe"]]]   # MSR as a single point with Sharpe Ratio
    
    # Step 10: Construct ECharts JSON
    sharpe_min = float(portfolios["sharpe"].min())          # Minimum Sharpe Ratio among all portfolios
    sharpe_max = float(portfolios["sharpe"].max())          # Maximum Sharpe Ratio among all portfolios
    ...
    plot_data = {
        "ef_plot": {
            ...
            "xAxis": {
                ...
                "name": "Volatility (%)",
                "min": 0.15,
                "max": vol_max * 1.1  # Added 10% buffer for dynamic scaling
            },
            "yAxis": {
                "type": "value",
                "name": "Return (%)",
                "min": 0.05,
                "max": ret_max * 1.1  # Added 10% buffer for dynamic scaling
            },
            # Configure visualMap to color-code portfolios based on Sharpe Ratio
            "visualMap": {
                "dimension": 2,                          # Uses the third element in scatter_data (Sharpe Ratio) for color mapping
                "min": sharpe_min,                       # Minimum Sharpe Ratio for the color scale
                "max": sharpe_max,                       # Maximum Sharpe Ratio for the color scale
                "inRange": {
                    "color": ["#0000FF", "#00FF00"]       # Blue to Green gradient
                    # Alternatively, uncomment the line below for Purple to Orange
                    # "color": ["#800080", "#FFA500"]    # Purple to Orange
                }
            },
            "series": {
                "Portfolios": scatter_data,            # Scatter plot data for all portfolios
                "Frontier":   line_data,               # Line plot data for the efficient frontier
                "GMV":        gmv_data,                # GMV as a single scatter point
                "MSR":        msr_data                 # MSR as a single scatter point
            }
        }
    }
    
    # Step 11: Output the Plot Data in JSON Format for ECharts Integration
    print("\n<ECHARTS_DATA>" + json.dumps(plot_data))
    
    # Step 12: Print summary of GMV / MSR in tabulate form
    summary_data = [
        ["GMV Portfolio",
