...
try:
    ...
    pd.DataFrame({'test': [1, 2, 3]})  # pandas
    np.array([1, 2, 3])  # numpy
    scipy.stats.norm.ppf(0.95)  # scipy.stats
    datetime.now()  # datetime
    pok.get_ffme_returns()  # PortfolioOptimizationKit
    ...
    for k_rho, rho in enumerate(rhos):
        ...
        for i in range(ww.shape[0]):
            ...
            portfolios.append([
                ...
                float(w2[i])  # For color mapping
            ])
        
        plot_data[f"ef_{k_rho}"] = {
            "type": "scatter",
            "yAxisName": "Return (%)",
            "title": f"Correlation: {rho:.2f}",
            "series": {
            ...
            "visualMap": {
                ...
                "dimension": 2,
                "inRange": {
                    "color": ["#FF4444", "#4444FF"]  # Red to Blue
                }
            }
        }
    
    print("Efficient Frontier Analysis")
    print(f"Correlations: {', '.join(f'{rho:.2f}' for rho in rhos)}")
...
In this section, we dig into the time series data of selected U.S. stocks and showcase effective methods for constructing optimized investment portfolios.
...
# Use your new function to retrieve the stock data from CSV
stocks = pok.get_stock_dynamic()

# (Optional) Round the stock data for better readability
...
# 1) Load the CSV-based stock data via your custom function
stocks = pok.get_stock_dynamic()

# 2) Calculate daily returns and annualized returns
daily_rets = pok.compute_returns(stocks)
ann_rets   = pok.annualize_rets(daily_rets, 252)

# 3) Compute mean, std, and covariance of daily returns
mean_rets = daily_rets.mean()
std_rets  = daily_rets.std()
cov_rets  = daily_rets.cov()

# 4) Define parameters for portfolio simulation
periods_per_year = 252
num_portfolios   = 4000
risk_free_rate   = 0.0

# Determine the number of assets from the columns of 'stocks'
...
for i in range(num_portfolios):
    ...
    all_portfolios.append({
        ...
        # Expand weights as needed; e.g. "w1", "w2", ...
        # If you have exactly 3 assets, you can name them w1, w2, w3
        "w1": weights[0],
        "w2": weights[1],
        "w3": weights[2] if n_assets >= 3 else None
    })

# 6) Convert all portfolio data into a DataFrame
...
def build_portfolio_frontier_json():
    """
    Generate random portfolios, build the efficient frontier,
    and output ECharts-friendly JSON (scatter + line).
    """
    # 1) Load the CSV-based stock data
    stocks = pok.get_stock_dynamic()

    # 2) Compute returns, vol, etc.
    daily_rets = pok.compute_returns(stocks)
    ann_rets   = pok.annualize_rets(daily_rets, 252)
    cov_rets   = daily_rets.cov()
    ...
    for _ in range(num_portfolios):
        ...
        port_vol = pok.annualize_vol(port_vol, periods_per_year)
        port_spr = pok.sharpe_ratio(port_ret, risk_free_rate, periods_per_year, v=port_vol)

        all_portfolios.append({
            "return":     float(port_ret),
            "volatility": float(port_vol),
            "sharpe":     float(port_spr)
        })
    portfolios = pd.DataFrame(all_portfolios)

    # 4) Build efficient frontier DataFrame
    df_frontier = pok.efficient_frontier(50, daily_rets, cov_rets, periods_per_year)
    # df_frontier has columns like ["volatility", "return"]

    # 5) Prepare scatter data for the portfolios:
    #    We'll store [volatility, return, sharpe] for each point.
    scatter_data = []
    for _, row in portfolios.iterrows():
        scatter_data.append([
            row["volatility"],   # dimension 0 => x-axis
            row["return"],       # dimension 1 => y-axis
            row["sharpe"]        # dimension 2 => color dimension
        ])

    # 6) Prepare line data for the frontier:
    #    We'll store [volatility, return] for each frontier point.
    line_data = []
    for _, row in df_frontier.iterrows():
        line_data.append([
            row["volatility"],   # x-axis
            row["return"]        # y-axis
        ])
    
    chart_key = "ef_plot"

    # Determine the min/max sharpe to set visualMap range
    ...
    plot_data = {
        chart_key: {
            ...
            "yAxis": {
                "type": "value",
                "min": 0,
                "max": 1.0,   # if your returns are <1 (i.e., 0.3 => 30%)
            },
            # The visualMap uses dimension=2 => the 3rd item in each data row => sharpe
            "visualMap": {
                "dimension": 2,
                "min": sharpe_min,
                "max": sharpe_max,
                "inRange": {
                    "color": ["#4444FF", "#FF4444"]  # Blue to Red
                }
            },
            "series": {
                "Portfolios": scatter_data,  # scatter
                "Frontier":   line_data      # line
            }
        }
    }
    # 8) Print as JSON for your Vue/ECharts "OutputDisplay.vue"
    print("\n<ECHARTS_DATA>" + json.dumps(plot_data))
...
The resulting scatter plot visualizes the distribution of portfolios based on their returns and volatility. The efficient frontier line represents the optimal balance between risk and return, guiding investors toward the most efficient investment choices.
...
def build_portfolios_with_gmv_msr():
    """
    Generate random portfolios, identify GMV & MSR, build the efficient frontier,
    print an ECharts-friendly JSON (scatter + line + single points),
    and output a table summarizing GMV and MSR stats.
    """
    # 1) Load CSV-based stock data
    stocks = pok.get_stock_dynamic()
    ...
    # 4) Identify GMV (lowest vol) and MSR (highest Sharpe)
    gmv_port = portfolios.iloc[portfolios["volatility"].idxmin()]
    msr_port = portfolios.iloc[portfolios["sharpe"].idxmax()]

    # 5) Build efficient frontier DataFrame
    df_frontier = pok.efficient_frontier(50, daily_rets, cov_rets, periods_per_year)

    # 6) Prepare scatter data for all random portfolios => [vol, ret, sharpe]
    scatter_data = []
    for _, row in portfolios.iterrows():
        scatter_data.append([
            row["volatility"],  
    ...
    # 7) Prepare line data for the frontier => [vol, ret]
    line_data = []
    for _, row in df_frontier.iterrows():
        line_data.append([row["volatility"], row["return"]])

    # 8) Single-point data for GMV & MSR => [[vol, ret]]
    gmv_data = [[gmv_port["volatility"], gmv_port["return"]]]
    msr_data = [[msr_port["volatility"], msr_port["return"]]]

    # 9) Construct ECharts JSON
    sharpe_min = float(portfolios["sharpe"].min())
    sharpe_max = float(portfolios["sharpe"].max())
    ...
    # 10) Print the JSON for Vue/ECharts
    print("\n<ECHARTS_DATA>" + json.dumps(plot_data))

    # 11) Print summary of GMV / MSR in tabulate form
    #     e.g. "Global Minimum Volatility portfolio"
    ...
    table_headers = ["Portfolio", "Return", "Volatility", "Sharpe Ratio"]
    table_str = tabulate(summary_data, headers=table_headers, tablefmt="github")
    print(table_str)
...
def monte_carlo_min_vol(daily_rets, cov_rets, n_sims=4000, risk_free_rate=0.0, periods_per_year=252):
    """
    Monte Carlo approach to randomly generate portfolios and
    return the GMV portfolio among them (lowest volatility).
    Returns a tuple: (ret, vol, sharpe).
    """
    ...
    for _ in range(n_sims):
        ...
        port_ret = pok.portfolio_return(weights, pok.annualize_rets(daily_rets, periods_per_year))
        port_vol = pok.portfolio_volatility(weights, cov_rets)
        port_vol = pok.annualize_vol(port_vol, periods_per_year)
        port_spr = pok.sharpe_ratio(port_ret, risk_free_rate, periods_per_year, v=port_vol)

        all_portfolios.append((port_ret, port_vol, port_spr))

    # Convert to DataFrame
    portfolios_df = pd.DataFrame(all_portfolios, columns=["return", "volatility", "sharpe"])

    # GMV from Monte Carlo
    gmv_row = portfolios_df.iloc[portfolios_df["volatility"].idxmin()]
    return (gmv_row["return"], gmv_row["volatility"], gmv_row["sharpe"])

def build_minvol_frontier_comparison(daily_rets, cov_rets, low_vol_portfolio, risk_free_rate=0.0, periods_per_year=252):
    """
    Show annual returns, find min-vol weights, build an ECharts JSON
    for the frontier & GMV portfolio, and compare Monte Carlo GMV vs. 
    analytical min-vol results.

    :param daily_rets:         DataFrame of daily returns
    :param cov_rets:           Covariance matrix of daily returns
    :param low_vol_portfolio:  tuple (ret, vol, sharpe) from your Monte Carlo approach
    :param risk_free_rate:     risk-free rate for Sharpe ratio computations
    :param periods_per_year:   trading days per year
    """
    # 1) Annualize returns
    ann_rets = pok.annualize_rets(daily_rets, periods_per_year)

    # Print annual returns (tabular)
    ann_rets_df = pd.DataFrame(ann_rets, columns=["Annual Return"])
    print("Annual Returns of Individual Assets:\n")
    print(tabulate(ann_rets_df * 100, headers=["Assets", "Annual Return (%)"], tablefmt="github"))
    print()

    # 2) Compute optimal weights for min volatility (analytical approach)
    optimal_weights = pok.minimize_volatility(ann_rets, cov_rets)

    # Print weights (assuming 3 assets: AMZN, KO, MSFT) - adjust if needed
    print("Optimal Weights (Min Vol Portfolio):")
    ticker_list = ["AMZN", "KO", "MSFT"]
    for i, ticker in enumerate(ticker_list):
        print(f"  {ticker}: {optimal_weights[i]*100:.2f}%")

    # 3) Compute portfolio features (ret, vol, sharpe) for the min-vol solution
    ret, vol, shp = get_portfolio_features(
        optimal_weights, ann_rets, cov_rets, risk_free_rate, periods_per_year
    )

    # 4) Build the frontier DataFrame
    df = pok.efficient_frontier(50, daily_rets, cov_rets, periods_per_year)

    # Convert the frontier to ECharts data => [vol, ret]
    frontier_data = df.apply(lambda r: [r["volatility"], r["return"]], axis=1).tolist()

    # Single point for the min-vol solution => [vol, ret]
    minvol_data = [[vol, ret]]

    # 5) Construct ECharts-friendly JSON
    echart_key = "minvol_plot"
    plot_data = {
        echart_key: {
            "type": "multi",
            "title": "Minimum Volatility Portfolio on Frontier",
            "yAxisName": "Return (%)",
            "xAxis": {
                "type": "value",
                "name": "Volatility (%)",
            ...
            # Two sub-series: "Frontier" => line, "MinVol" => single scatter point
            "series": {
    ...
    # 6) Compare Monte Carlo GMV (low_vol_portfolio) vs. analytical MinVol
    print("\nGlobal Minimum Volatility (GMV) portfolio comparison:\n")
    summary_data = [
        ["Monte Carlo GMV",
         f"{low_vol_portfolio[0]*100:.2f}%",
         f"{low_vol_portfolio[1]*100:.2f}%",
         f"{low_vol_portfolio[2]:.2f}"],
        ["Min. Vol (Opt)",
         f"{ret*100:.2f}%",
         f"{vol*100:.2f}%",
         f"{shp:.2f}"]
    ]
    table_headers = ["Approach", "Return", "Volatility", "Sharpe"]
    print(tabulate(summary_data, headers=table_headers, tablefmt="github"))
...
if __name__ == "__main__":
    ...
    # 2) Monte Carlo approach for GMV
    #    If you want, you can call the helper function or do inline
    low_vol_portfolio = monte_carlo_min_vol(daily_rets, cov_rets, n_sims=4000)

    # 3) Build min-vol frontier comparison
    build_minvol_frontier_comparison(daily_rets, cov_rets, low_vol_portfolio)
```
