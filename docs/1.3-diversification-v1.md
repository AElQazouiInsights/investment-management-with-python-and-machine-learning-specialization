...
# Convert the moving averages to lists (helper defined earlier).
...
# Helper defined in the Utility Helpers section converts NaN to None.
...
# Add or remove sectors by editing the dictionary below—each entry shares the
# same scaffold, so you can loop over a list if you prefer to generate charts
# programmatically for a larger universe.
charts = {}
...
# Helper from the Utility Helpers section converts NaN to None.
...
# Helpers `convert_nan_to_none` and `df_to_series_dict` are defined in the Utility Helpers section.
