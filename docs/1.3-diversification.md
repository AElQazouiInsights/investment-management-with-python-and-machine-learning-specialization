...
```python:line-numbers
gbm_controls = widgets.interact(pok.show_gbm_echart, 
...
This segment introduces the **`show_cppi`** method from the **`pok`** toolkit, which simulates a Constant Proportion Portfolio Insurance (CPPI) investment strategy based on returns generated by Geometric Brownian Motion. It focuses on a fixed-floor scenario, specifically without a drawdown constraint. For in-browser interactivity we expose a companion helper, **`show_cppi_echart`**, that streams the same simulation as an ECharts payload.
...
widget_instance = widgets.interact(
    ...
    n_years=(1, 10, 1),          # Number of years in the simulation
    n_scenarios=(1, 300, 1),     # How many Monte Carlo paths to generate
    m=(1.0, 6.0, 0.5),           # CPPI multiplier
    floor=(0.0, 1.0, 0.05),      # Floor as a fraction of current wealth
    mu=(-0.20, 0.40, 0.01),      # Drift of the risky asset
    sigma=(0.0, 0.50, 0.01),     # Volatility of the risky asset
    risk_free_rate=(0.01, 0.05, 0.01),  # Safe-asset rate
    periods_per_year=[12, 52, 252],     # Rebalancing frequency
    start=[100],                 # Initial price/wealth
