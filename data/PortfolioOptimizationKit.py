...
def efficient_frontier(
    ...
    """
    Returns a dataframe 'df' containing volatilities, returns, sharpe ratios, and weights
    for 'n_portfolios' along the efficient frontier.
    If cml=True, also returns 'cml_df' with two points describing the Capital Market Line:
       (0, risk_free_rate) -> (vol_sharpe, ret_sharpe).
    If iplot=True, the function also plots with Matplotlib:
       - The Frontier (in coral),
       - The optional CML (in green dotted),
       - The highest Sharpe ratio portfolio (HSR) if hsr=True,
       - The global minimum volatility portfolio if mvp=True,
       - The equally weighted portfolio if ewp=True.

    Inputs:
      n_portfolios: int
      rets: DataFrame of historical returns (e.g., daily, weekly, monthly) for each asset
      covmat: Covariance matrix of those returns
      periods_per_year: e.g., 12 (monthly), 52 (weekly), 252 (daily)
      risk_free_rate: e.g., 0.06 => 6%
      iplot: bool => whether to do a Matplotlib plot
      hsr, cml, mvp, ewp => booleans controlling whether to mark certain special portfolios
    """

    def append_row_df(df, vol, ret, spr, weights):
        """
        Utility function that appends a single row [vol, ret, spr, w0, w1, ...]
        to the DataFrame 'df' with the same columns.
        """
    ...
    # 1) Generate frontier portfolio weights
    weights = optimal_weights(n_portfolios, ann_rets, covmat, periods_per_year)
    # If your portfolio is just 2 assets, you could do a simple np.linspace(0,1,n_portfolios).

    # 2) Compute vol/ret/spr for each set of weights
    ...
    # 3) Build the Frontier DataFrame
    #    columns => ["volatility","return","sharpe ratio", weight0, weight1, ...]
    df = pd.DataFrame({
    ...
    # 4) If cml=True, also compute the tangency (max Sharpe) portfolio => 2 points for the CML
    cml_df = None
    if cml:
        ...
        # cml_df => columns: ["volatility","return","sharpe ratio"]
        cml_points = [
            [0.0, risk_free_rate, 0.0],          # (vol=0, ret=Rf)
            [vol_sharpe, ret_sharpe, spr_sharpe] # tangency portfolio
        ]
        cml_df = pd.DataFrame(cml_points, columns=["volatility","return","sharpe ratio"])

    # 5) If iplot=True => do the old Matplotlib plotting
    if iplot:
        ax = df.plot.line(
            x="volatility",
            y="return",
        ...
        # If hsr or cml => find tangency portfolio
        if hsr or cml:
            w = maximize_sharpe_ratio(ann_rets, covmat, risk_free_rate, periods_per_year)
            ret_ = portfolio_return(w, ann_rets)
            vol_ = annualize_vol(portfolio_volatility(w, covmat), periods_per_year)
            spr_ = sharpe_ratio(ret_, risk_free_rate, periods_per_year, v=vol_)

            # Optionally append that row to df
            df = append_row_df(df, vol_, ret_, spr_, w)

            # If cml => draw the line from (0,Rf) to (vol,ret)
            if cml:
                ax.plot(
                    [0, vol_],
                    [risk_free_rate, ret_],
            ...
            # If hsr => scatter that point
            if hsr:
                ax.scatter([vol_], [ret_], marker="o", color="g", label="MSR portfolio")
                ax.legend()

        # 6) If global min vol => mvp=True => plot that
        if mvp:
            ...
            df = append_row_df(df, vol_, ret_, spr_, w)
            ax.scatter(
                [vol_], [ret_], color="midnightblue", marker="o", label="GMV portfolio"
            )
            ax.legend()

        # 7) If equally weighted => ewp=True
        if ewp:
            w = np.repeat(1.0 / ann_rets.shape[0], ann_rets.shape[0])
            ret_ = portfolio_return(w, ann_rets)
            vol_ = annualize_vol(portfolio_volatility(w, covmat), periods_per_year)
            spr_ = sharpe_ratio(ret_, risk_free_rate, periods_per_year, v=vol_)
            df = append_row_df(df, vol_, ret_, spr_, w)
            ax.scatter(
                [vol_], [ret_], color="goldenrod", marker="o", label="EW portfolio"
            )
            ax.legend()

        return df, ax
    else:
        # If not plotting => just return the Frontier DataFrame
        # plus the cml_df if cml=True
...
def cppi(
    ...
    for step in range(len(risky_rets.index)):
        ...
        account_value = risky_allocation * (
            1 + risky_rets.iloc[step]
        ) + safe_allocation * (1 + safe_rets.iloc[step])
    ...
    if drawdown is not None:
        backtest_result.update(
            {"Floor value": floor_history, "Peaks": peak_history, "m": m}
        )
...
def show_gbm(
    ...
    """
    prices, rets = simulate_gbm_from_returns(
        n_years=n_years,
...
def show_cppi(
    ...
    cppiw.plot(
        ax=wealth_ax, grid=True, legend=False, color=simclr, alpha=0.5, linewidth=2
    )
    ...
    wealth_ax.set_title(
        "CPPI wealths due to brownian motion generated returns", fontsize=14
    )

    # Plot the histogram
    violations_per_scenarios = (
        cppiw < start * floor
    ).sum()  # number of CPPI wealth violations of the floor per each scenario
    total_violations = (
        violations_per_scenarios.sum()
    )  # overall number of CPPI wealth violations during the entire period
    ...
    terminal_wealth.hist(
        grid=False, ax=hist_ax, bins=50, ec="white", fc=simclr, orientation="horizontal"
    )
    ...
    hist_ax.annotate(
        "Mean: ${:.2f}".format(tw_mean),
        xy=(0.5, 0.9),
        xycoords="axes fraction",
        fontsize=15,
    )
    hist_ax.annotate(
        "Median: ${:.2f}".format(tw_mean),
        xy=(0.5, 0.85),
        xycoords="axes fraction",
        fontsize=15,
    )
    if floor > 0.0:
        hist_ax.annotate(
            "Violations (overall): {}".format(total_violations),
            xy=(0.5, 0.75),
            xycoords="axes fraction",
            fontsize=15,
        )
        hist_ax.annotate(
            "Violations (end period): {} ({:.1f}%)".format(n_failures, p_fail * 100),
            xy=(0.5, 0.7),
            xycoords="axes fraction",
            fontsize=15,
        )
        hist_ax.annotate(
            "E(shortfall) (end period): ${:.2f}".format(e_shorfall),
            xy=(0.5, 0.65),
            xycoords="axes fraction",
            fontsize=15,
        )
    hist_ax.set_title(
        "Histogram of the CPPI wealth at the end of the period", fontsize=14
    )
...
def simulate_cir(
    n_years=10, n_scenarios=10, a=0.05, b=0.03, sigma=0.05, periods_per_year=12, r0=None
):
    """
    ...
    def zcbprice(ttm, r, h):
        A = (
            (2 * h * np.exp(0.5 * (a + h) * ttm))
            / (2 * h + (a + h) * (np.exp(h * ttm) - 1))
        ) ** (2 * a * b / (sigma**2))
...
def bond_cash_flows(principal=100, maturity=10, coupon_rate=0.03, coupons_per_year=2):
    ...
    cash_flows = pd.DataFrame(
        coupon_amount, index=np.arange(1, n_coupons + 1), columns=[0]
    )
...
def bond_price(
    principal=100, maturity=10, coupon_rate=0.02, coupons_per_year=2, ytm=0.03, cf=None
):
    """
    ...
    if isinstance(ytm, pd.DataFrame):
        ...
        for i in range(n_scenarios):
            ...
            prices = [
                single_price_bond(
                    principal=principal,
                    maturity=maturity - t / coupons_per_year,
                    coupon_rate=coupon_rate,
                    coupons_per_year=coupons_per_year,
                    ytm=y,
                    cf=cf,
                )
                for t, y in zip(ytm.index[:-1], ytm.iloc[:-1, i])
            ]
        ...
        bond_price = pd.concat(
            [
                bond_price,
                pd.DataFrame(
                    [
                        [principal + principal * coupon_rate / coupons_per_year]
                        * n_scenarios
                    ],
                    index=[ytm.index[-1]],
                ),
...
def bond_returns(
    ...
    if isinstance(bond_prices, pd.DataFrame):
        coupons = pd.DataFrame(
            data=0, index=bond_prices.index, columns=bond_prices.columns
        )
        last_date = bond_prices.index.max()
        pay_date = np.linspace(
            periods_per_year / coupons_per_year,
            last_date,
            int(coupons_per_year * last_date / periods_per_year),
            dtype=int,
        )
...
def ldi_floor_allocator(psp_rets, lhp_rets, zcb_price, floor, m=3):
    ...
    for date in range(dates):
        ...
        lhp_w = 1 - psp_w
        # update
        account_value = psp_w * account_value * (
            1 + psp_rets.iloc[date]
        ) + lhp_w * account_value * (1 + lhp_rets.iloc[date])
...
def linear_regression(dep_var, exp_vars, alpha=True):
    ...
    if alpha:
        ...
        if isinstance(exp_vars, pd.DataFrame):
            ...
            exp_vars = np.concatenate(
                (exp_vars, np.ones((exp_vars.shape[0], 1))), axis=1
            )
...
def style_analysis(dep_var, exp_vars):
    ...
    weights_const = {"type": "eq", "fun": lambda weights: 1 - np.sum(weights)}
    solution = minimize(
        style_analysis_tracking_error,
        init_guess,
        method="SLSQP",
        options={"disp": False},
...
def weight_maxsharpe(
    r, cov_estimator=sample_cov, periods_per_year=12, risk_free_rate=0.03, **kwargs
):
    """
...
def implied_returns(covmat, weigths, delta=2.5):
    ...
    """
    imp_rets = (
        delta * covmat.dot(weigths).squeeze()
...
def omega_uncertain_prior(covmat, tau, P):
    ...
    return pd.DataFrame(
        np.diag(np.diag(he_lit_omega.values)), index=P.index, columns=P.index
    )
...
def black_litterman(w_prior, Sigma_prior, P, Q, Omega=None, delta=2.5, tau=0.02):
    ...
    sigma_bl = (
        Sigma_prior
        + (tau * Sigma_prior)
        - (tau * Sigma_prior).dot(P.T).dot(invmat).dot(P).dot(tau * Sigma_prior)
    )
...
def risk_parity_weigths(mat_cov):
    ...
    """
    n = mat_cov.shape[0]
    weigths = portfolio_risk_contrib_optimizer(
        target_risk_contrib=np.repeat(1 / n, n), mat_cov=mat_cov
    )
